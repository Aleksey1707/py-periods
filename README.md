...
В примерах я использую наглядное представление периодов и дат.
Периоды (DatePeriod) я изображаю в виде интервала: |=======|
Дату (datetime, date) я изображаю в виде точки: +
...
Подробное описание:
    ...
    Исключение ValueError вызывается в случае, если я что-то не предусмотрел.
```
...
## 15. Сортировка
```
Из-за того, что методы сравнения переопределены и работают "интересным" способом, а также из-за того, что я
просто в свое время не подумал на этот счет, то при сортировке списка объектов DatePeriod, методы sort и sorted
без переданных пользовательских функций key или cmp, отсортируют список НЕ ВЕРНО.

Для таких случаев можно использовать следующие способы:
    1) Использование key
    sorted([p1, p2, p3, p4, p5], key=lambda x: x.begin, x.end)
    или
    [p1, p2, p3, p4, p5].sort(key=lambda x: x.begin, x.end)

P.S.: Возможно правильнее было бы функционал переопределенных операций сравнения описать с помощью обычных методов,
но мне было комфортнее (быстрее и нагляднее) работать с периодами именно через переопределенные операции сравнения.
Ради "нюанса" с сортировкой, о которой я в свое время даже и не думал заморачиваться смысла я не вижу.
Пусть будет так, как сделано.
```
